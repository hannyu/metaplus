...
public class DocUtil {
    ...
    public static List<String[]> diffDoc(MetaplusDoc leftDoc, MetaplusDoc rightDoc) {
        List<String[]> diffs = JsonDiff.diff(leftDoc, rightDoc);
        diffs = diffs.stream().filter(dif -> {
            return !dif[0].startsWith("$.fqmn.") && !dif[0].startsWith("$.sync.");
        }).toList();
        return diffs;
    }
